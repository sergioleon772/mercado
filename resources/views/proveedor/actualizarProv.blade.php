    ...
    <form action="{{ url('/proveedor/' . $proveedor->id) }}" class="container mt-5 mb-5 w-50" method="POST">
        ...
        <div class="row g-3">
            ...
            <div class="col-6">
                ...
                <input type="email" value="{{ $proveedor->correo }}" class="form-control" name="correo" id="correoInput"
                    required>
            </div>
            <div class="col-6">
                <label for="telefonoInput" class="form-label">Teléfono</label>
                <input type="number" value="{{ $proveedor->telefono }}" class="form-control" name="telefono"
                    id="telefonoInput" required>
            </div>
            <div class="col-12">
                <label for="direccionInput" class="form-label">Dirección</label>
                <input type="text" id="address" value="{{ $proveedor->direccion }}" class="form-control"
                    name="direccion" id="direccionInput" required>
            </div>

            <div class="col-12">
                <label for="productosInput" class="form-label">Productos a Comerciar</label>
                <textarea name="productos_a_comerciar" class="form-control @error('productos_a_comerciar') is-invalid @enderror"
                    id="productosInput" cols="30" rows="3">{{ old('productos_a_comerciar', $proveedor->productos_a_comerciar) }}</textarea>
